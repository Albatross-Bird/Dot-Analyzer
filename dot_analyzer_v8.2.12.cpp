/*
 * Dot Analyzer v8.2.12 — ImGui + OpenGL3 Edition
 *
...
/* Smallest angle in [0, pi/2] where the rotated label's horizontal projection
   fits within bar_slot pixels (with a 4px gap). */
static double compute_label_angle(int lbl_w, int bar_slot) {
    static const double DEG = 3.14159265358979323846 / 180.0;
    if (lbl_w + 4 <= bar_slot) return 0.0;
    for (int deg = 1; deg <= 90; deg++) {
        double rad  = deg * DEG;
        double span = lbl_w * cos(rad) + PFONT_H * sin(rad);
        if (span + 4.0 <= (double)bar_slot) return rad;
    }
    return 90.0 * DEG;
}

/* Render string rotated CCW by angle_rad, anchored at (ax, ay) = top-right corner
   of the unrotated string.  Text extends down-left; reading direction is
   lower-left -> anchor (upper-right), matching Excel-style rotated axis labels. */
static void pstr_rot(uint8_t *rgb, int ax, int ay, const char *s, double angle_rad,
                     uint8_t r, uint8_t g, uint8_t b) {
    ...
    for (const char *p = s; *p; p++, ci++) {
        ...
        for (int row = 0; row < PFONT_H; row++) {
            ...
            for (int col = 0; col < PFONT_W; col++) {
                ...
                /* Pixel position in the unrotated string, origin = top-right corner */
                double dx = (double)(ci * (PFONT_W + 1) + col) - total_w;
                double dy = (double)row;
                /* CCW rotation in y-down screen coordinates */
                int rx = (int)round(dx * cos_a + dy * sin_a);
                int ry = (int)round(-dx * sin_a + dy * cos_a);
                ppx(rgb, ax + rx, ay + ry, r, g, b);
            }
        }
    }
}
...
static uint8_t *render_dist_rgb(double bw_override) {
    ...
    if (max_count == 0) { free(counts); return NULL; }

    /* Y-axis ceiling: tallest bar at 75% of plot height */
    double y_axis_max = max_count / 0.75;

    /* --- Compute x-axis label rotation angle.
       pw doesn't depend on MB, so the angle is determined before canvas layout.
       MB is then set to fit the vertical drop of the rotated labels. --- */
    ...
    int bar_slot_px  = pw_fixed / (nbins > 0 ? nbins : 1);
    double rot_angle = compute_label_angle(lbl_w_sample, bar_slot_px);
    /* Vertical drop below the tick anchor = lbl_w*sin(rot) + PFONT_H*cos(rot) */
    int mb_needed = (int)ceil(lbl_w_sample * sin(rot_angle) + PFONT_H * cos(rot_angle)) + 16;
    const int MB  = mb_needed > 52 ? mb_needed : 52;

    /* --- Allocate 800x480 RGB canvas --- */
    uint8_t *rgb = (uint8_t*)malloc(PLOT_W * PLOT_H * 3);
    if (!rgb) { free(counts); return NULL; }
    ...
    /* --- X-axis ticks: one label per bin, centered on bar.
       Median bin drawn in red. Labels rotate CCW when bins are too narrow for horizontal text.
       Rotated labels anchor their top-right at the tick; text reads lower-left -> tick. --- */
    for (int i = 0; i < nbins; i++) {
        ...
        int lw = pstr_w(lbl);
        if (rot_angle > 0.001)
            pstr_rot(rgb, tx, py1 + tick_len + 2, lbl, rot_angle, tr, tg, tb);
        else
...
static void draw_ui(void) {
    ...
    if (ImGui::BeginPopupModal("##distprev", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        ImGui::SameLine();
        if (ImGui::Button("Close##dprev")) ImGui::CloseCurrentPopup();
        ImGui::EndPopup();
    }
