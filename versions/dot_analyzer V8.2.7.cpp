/*
 * Dot Analyzer v8.2.7 — ImGui + OpenGL3 Edition
 *
...
static int blob_sort_cmp(const void *a, const void *b);  /* forward decl — defined before process_images */
...
static void build_preview_rgb(void) {
    ...
    /* Bounding boxes */
    int first_idx = -1;
    if (g_out_csv && g_preview_nblobs > 0) {
        first_idx = 0;
        for (int bi = 1; bi < g_preview_nblobs; bi++) {
            if (blob_sort_cmp(&g_preview_blobs[bi], &g_preview_blobs[first_idx]) < 0)
                first_idx = bi;
        }
    }
    for (int bi = 0; bi < g_preview_nblobs; bi++) {
...
/* ── CSV sort comparator ─────────────────────────────────────────────── */
static int blob_sort_cmp(const void *a, const void *b) {
    ...
    int pm = g_csv_sort_primary ? -1 : 1;
    int sm = g_csv_sort_second  ? -1 : 1;
    if (pA != pB) return pm * (pA - pB);
    return sm * (sA - sB);
}
...
static void process_images(void) {
    ...
    for (int fi = 0; fi < nfiles; fi++) {
        ...
        if (g_out_csv && nb > 1)
            qsort(blobs, nb, sizeof(blob_t), blob_sort_cmp);
