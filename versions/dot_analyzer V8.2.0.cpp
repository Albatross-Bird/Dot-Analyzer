/*
 * Dot Analyzer v8.2.0 — ImGui + OpenGL3 Edition
 *
...
static void draw_ui(void) {
    ...
    ImGui::BeginChild("##LeftPanel", ImVec2(g_left_panel_w, total_h), true);
    {
        ...
        if (ImGui::CollapsingHeader(L->sec_analyze)) {

            /* -- Image Folder -- */
            ImGui::SetNextItemOpen(true, ImGuiCond_Once);
            if (ImGui::CollapsingHeader(L->sec_image_folder)) {
                ImGui::Spacing();
            ...
            if (ImGui::CollapsingHeader(L->sec_calibration)) {
                ImGui::Spacing();
            ...
            /* -- Threshold -- */
            ImGui::SetNextItemOpen(false, ImGuiCond_Once);
            if (ImGui::CollapsingHeader(L->sec_threshold)) {
                ImGui::Spacing();
            ...
            /* -- Overlays -- */
            ImGui::SetNextItemOpen(false, ImGuiCond_Once);
            if (ImGui::CollapsingHeader(L->sec_overlays)) {
                ImGui::Spacing();
        ...
        ImGui::Spacing();
        ImGui::SetNextItemOpen(false, ImGuiCond_Once);
        if (ImGui::CollapsingHeader(L->sec_record_mode)) {
            ImGui::Spacing();
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int nShow) {
    ...
    g_hwnd = CreateWindowW(L"DotAnalyzerImGui", L"Dot Analyzer  v8.2.0",
