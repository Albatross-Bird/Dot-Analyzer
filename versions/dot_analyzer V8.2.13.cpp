/*
 * Dot Analyzer v8.2.13 — ImGui + OpenGL3 Edition
 *
...
static uint8_t *render_dist_rgb(double bw_override) {
    ...
    /* --- X-axis ticks: one label per bin, centered on bar.
       Median bin drawn in red.  When bins are too narrow for horizontal text the
       label is rendered 90 deg CCW: reads bottom-to-top, last char at the tick. --- */
    for (int i = 0; i < nbins; i++) {
        ...
        int lw = pstr_w(lbl);
        if (vert_labels)
            pstr_vert(rgb, tx - PFONT_H / 2, py1 + tick_len + 2, lbl, tr, tg, tb);
        else
