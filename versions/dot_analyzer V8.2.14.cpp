/*
 * Dot Analyzer v8.2.14 — ImGui + OpenGL3 Edition
 *
...
static uint8_t *render_dist_rgb(double bw_override) {
    ...
    /* Narrow percentile bounds so regular bins are built within the clipped range */
    if (has_clip_lo && clip_lo > p_lo) p_lo = clip_lo;
    if (has_clip_hi && clip_hi < p_hi) p_hi = clip_hi;
    ...
    int under_count = 0, over_count = 0;
    for (int i = 0; i < g_xnd; i++) {
        double v = g_xdiams[i];
        if (has_clip_lo && v < clip_lo) { under_count++; continue; }
        if (has_clip_hi && v > clip_hi) { over_count++;  continue; }
        int b = (int)((v - bmin) / bw);
        if (b < 0) b = 0;
        if (b >= nbins) b = nbins - 1;
        counts[b]++;
    }
    /* Build extended array: [underflow?] [regular bins...] [overflow?] */
    int total_bins = has_clip_lo + nbins + has_clip_hi;
    int *all_counts = (int*)calloc(total_bins, sizeof(int));
    if (!all_counts) { free(counts); return NULL; }
    if (has_clip_lo) all_counts[0] = under_count;
    for (int i = 0; i < nbins; i++) all_counts[has_clip_lo + i] = counts[i];
    if (has_clip_hi) all_counts[total_bins - 1] = over_count;
    free(counts);
    int max_count = 0;
    ...
    int vert_labels  = (lbl_w_sample + 4 > bar_slot_px);
    ...
    /* --- Bars: steel blue for regular bins, amber for under/overflow --- */
    ...
    for (int i = 0; i < total_bins; i++) {
        ...
        if (bar_h < 1) bar_h = 1;
        int is_clip_bin = (has_clip_lo && i == 0) || (has_clip_hi && i == total_bins - 1);
        if (is_clip_bin)
            prect_fill(rgb, bx0, py1 - bar_h, bx1 - 1, py1 - 1, 210, 130, 40);
        else
    ...
    /* --- X-axis ticks: one label per bin, centered on bar.
       Median bin drawn in red.  Under/overflow bins drawn in amber.
       When bins are too narrow for horizontal text the label is rendered 90 deg CCW. --- */
    for (int i = 0; i < total_bins; i++) {
        int j = i - has_clip_lo;  /* index into regular bins */
        int is_under = (has_clip_lo && i == 0);
        int is_over  = (has_clip_hi && i == total_bins - 1);
        int tx = px0 + (int)(((i + 0.5) / total_bins) * pw + 0.5);
        if (tx <= px0 || tx >= px1) continue;
        int is_median_bin = (!is_under && !is_over && j == half_steps);
        int tr, tg, tb_c;
        if (is_median_bin)         { tr = 200; tg =  40; tb_c =  40; }
        else if (is_under || is_over) { tr = 180; tg =  90; tb_c =  20; }
        else                       { tr =  60; tg =  60; tb_c =  60; }
        int tick_len = is_median_bin ? 6 : 4;
        pline_v(rgb, tx, py1, py1 + tick_len, tr, tg, tb_c);
        char lbl[16];
        if (is_under)      snprintf(lbl, sizeof(lbl), "< %.3f", clip_lo);
        else if (is_over)  snprintf(lbl, sizeof(lbl), "> %.3f", clip_hi);
        else               snprintf(lbl, sizeof(lbl), "%.3f", bmin + j * bw);
        int lw = pstr_w(lbl);
        if (vert_labels)
...
/* --- Write distribution PNG to disk --- */
...
static void draw_ui(void) {
    ...
    if (ImGui::BeginPopupModal("##distprev", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        ImGui::Separator();
        bool no_data = (g_xnd < 2);
        ImGui::Checkbox("Auto bin size##dprev", (bool*)&g_dist_bin_auto);
        if (!g_dist_bin_auto) {
            ImGui::SameLine();
            ImGui::SetNextItemWidth(80 * g_dpi_scale);
        ...
        ImGui::InputText("Min##dcliplo", g_dist_clip_min_buf, sizeof(g_dist_clip_min_buf));
        ImGui::SameLine();
        ImGui::SetNextItemWidth(70.0f * g_dpi_scale);
        ImGui::InputText("Max##dcliphi", g_dist_clip_max_buf, sizeof(g_dist_clip_max_buf));
        ImGui::SameLine();
        if (no_data) ImGui::BeginDisabled();
