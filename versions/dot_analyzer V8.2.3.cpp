/*
 * Dot Analyzer v8.2.3 — ImGui + OpenGL3 Edition
 *
...
/* ================================================================
 *  SECTION 14b — XLSX OUTPUT  (store-method ZIP + OOXML, no dependencies)
 * ================================================================ */
...
typedef struct { char *p; size_t len; size_t cap; } sb_t;
static void sb_append(sb_t *sb, const char *s) {
    size_t sl = strlen(s);
    if (sb->len + sl + 1 > sb->cap) {
        sb->cap = (sb->len + sl + 1) * 2 + 512;
        sb->p = (char*)realloc(sb->p, sb->cap);
    }
    memcpy(sb->p + sb->len, s, sl);
    sb->len += sl;
    sb->p[sb->len] = '\0';
}
static void sb_free(sb_t *sb) { free(sb->p); sb->p = NULL; sb->len = sb->cap = 0; }

/* --- ZIP store-method writer --- */
typedef struct { FILE *f; uint8_t *cd; size_t cd_len, cd_cap; uint16_t entries; } zipw_t;
static void zle16(uint8_t *p, uint16_t v) { p[0]=v&0xFF; p[1]=(v>>8)&0xFF; }
static void zle32(uint8_t *p, uint32_t v) { p[0]=v&0xFF; p[1]=(v>>8)&0xFF; p[2]=(v>>16)&0xFF; p[3]=(v>>24)&0xFF; }
static void zipw_add(zipw_t *z, const char *name, const uint8_t *data, size_t dlen) {
    uint32_t crc  = crc32_calc(data, dlen);
    uint16_t nlen = (uint16_t)strlen(name);
    uint32_t off  = (uint32_t)ftell(z->f);
    uint8_t lh[30]; memset(lh,0,30);
    zle32(lh+ 0, 0x04034b50u); zle16(lh+ 4, 20);
    zle32(lh+14, crc); zle32(lh+18,(uint32_t)dlen); zle32(lh+22,(uint32_t)dlen);
    zle16(lh+26, nlen);
    fwrite(lh,1,30,z->f); fwrite(name,1,nlen,z->f); fwrite(data,1,dlen,z->f);
    size_t need = z->cd_len + 46 + nlen;
    if (need > z->cd_cap) { z->cd_cap = need*2+512; z->cd = (uint8_t*)realloc(z->cd, z->cd_cap); }
    uint8_t *ce = z->cd + z->cd_len; memset(ce,0,46+nlen);
    zle32(ce+ 0, 0x02014b50u); zle16(ce+4,20); zle16(ce+6,20);
    zle32(ce+16, crc); zle32(ce+20,(uint32_t)dlen); zle32(ce+24,(uint32_t)dlen);
    zle16(ce+28, nlen); zle32(ce+42, off); memcpy(ce+46, name, nlen);
    z->cd_len += 46+nlen; z->entries++;
}
static void zipw_finish(zipw_t *z) {
    uint32_t cd_off = (uint32_t)ftell(z->f);
    fwrite(z->cd,1,z->cd_len,z->f);
    uint8_t eocd[22]; memset(eocd,0,22);
    zle32(eocd+0,0x06054b50u);
    zle16(eocd+8,z->entries); zle16(eocd+10,z->entries);
    zle32(eocd+12,(uint32_t)z->cd_len); zle32(eocd+16,cd_off);
    fwrite(eocd,1,22,z->f);
    free(z->cd); fclose(z->f);
}

/* --- Diameter collection for xlsx --- */
...
#define XLSX_BINS 20
typedef struct { double lo, hi; int count; } xbin_t;
static void calc_hist(xbin_t bins[XLSX_BINS]) {
    double mn=g_xdiams[0], mx=g_xdiams[0];
    for (int i=1;i<g_xnd;i++) { if(g_xdiams[i]<mn)mn=g_xdiams[i]; if(g_xdiams[i]>mx)mx=g_xdiams[i]; }
    double rng = mx-mn; if (rng<1e-9) rng=1e-9;
    double bw = rng/XLSX_BINS;
    for (int i=0;i<XLSX_BINS;i++) { bins[i].lo=mn+i*bw; bins[i].hi=mn+(i+1)*bw; bins[i].count=0; }
    for (int i=0;i<g_xnd;i++) { int b=(int)((g_xdiams[i]-mn)/bw); if(b>=XLSX_BINS)b=XLSX_BINS-1; if(b<0)b=0; bins[b].count++; }
}

/* --- xlsx XML parts --- */
static void xlsx_content_types(sb_t *sb) {
    ...
    sb_append(sb,"<Override PartName=\"/xl/worksheets/sheet1.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml\"/>");
    sb_append(sb,"<Override PartName=\"/xl/worksheets/sheet2.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml\"/>");
    sb_append(sb,"<Override PartName=\"/xl/styles.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml\"/>");
    sb_append(sb,"<Override PartName=\"/xl/drawings/drawing1.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.drawing+xml\"/>");
    sb_append(sb,"<Override PartName=\"/xl/charts/chart1.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.drawingml.chart+xml\"/>");
    sb_append(sb,"</Types>");
}
static void xlsx_root_rels(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">");
    sb_append(sb,"<Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument\" Target=\"xl/workbook.xml\"/>");
    sb_append(sb,"</Relationships>");
}
static void xlsx_workbook(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<workbook xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\">");
    sb_append(sb,"<sheets>");
    sb_append(sb,"<sheet name=\"Data\" sheetId=\"1\" r:id=\"rId1\"/>");
    sb_append(sb,"<sheet name=\"Distribution\" sheetId=\"2\" r:id=\"rId2\"/>");
    sb_append(sb,"</sheets></workbook>");
}
static void xlsx_workbook_rels(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">");
    sb_append(sb,"<Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet\" Target=\"worksheets/sheet1.xml\"/>");
    sb_append(sb,"<Relationship Id=\"rId2\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet\" Target=\"worksheets/sheet2.xml\"/>");
    sb_append(sb,"<Relationship Id=\"rId3\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles\" Target=\"styles.xml\"/>");
    sb_append(sb,"</Relationships>");
}
static void xlsx_styles(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<styleSheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\">");
    sb_append(sb,"<fonts count=\"1\"><font><sz val=\"11\"/><name val=\"Calibri\"/></font></fonts>");
    sb_append(sb,"<fills count=\"2\"><fill><patternFill patternType=\"none\"/></fill><fill><patternFill patternType=\"gray125\"/></fill></fills>");
    sb_append(sb,"<borders count=\"1\"><border><left/><right/><top/><bottom/><diagonal/></border></borders>");
    sb_append(sb,"<cellStyleXfs count=\"1\"><xf numFmtId=\"0\" fontId=\"0\" fillId=\"0\" borderId=\"0\"/></cellStyleXfs>");
    sb_append(sb,"<cellXfs count=\"2\">");
    sb_append(sb,"<xf numFmtId=\"0\" fontId=\"0\" fillId=\"0\" borderId=\"0\" xfId=\"0\"/>");
    sb_append(sb,"<xf numFmtId=\"4\" fontId=\"0\" fillId=\"0\" borderId=\"0\" xfId=\"0\"/>");
    sb_append(sb,"</cellXfs></styleSheet>");
}
static void xlsx_sheet1(sb_t *sb) {
    char tmp[128];
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>");
    sb_append(sb,"<row r=\"1\"><c r=\"A1\" t=\"inlineStr\"><is><t>Diameter_mm</t></is></c></row>");
    for (int i=0;i<g_xnd;i++) {
        snprintf(tmp,sizeof(tmp),"<row r=\"%d\"><c r=\"A%d\" s=\"1\"><v>%.6f</v></c></row>",i+2,i+2,g_xdiams[i]);
        sb_append(sb,tmp);
    }
    sb_append(sb,"</sheetData></worksheet>");
}
static void xlsx_sheet2(sb_t *sb, const xbin_t bins[XLSX_BINS]) {
    char tmp[256];
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\"><sheetData>");
    sb_append(sb,"<row r=\"1\">");
    sb_append(sb,"<c r=\"A1\" t=\"inlineStr\"><is><t>Bin (mm)</t></is></c>");
    sb_append(sb,"<c r=\"B1\" t=\"inlineStr\"><is><t>Count</t></is></c>");
    sb_append(sb,"<c r=\"C1\" t=\"inlineStr\"><is><t>Percent</t></is></c>");
    sb_append(sb,"</row>");
    for (int i=0;i<XLSX_BINS;i++) {
        double pct = (g_xnd>0) ? (bins[i].count*100.0/g_xnd) : 0.0;
        char lbl[64]; snprintf(lbl,sizeof(lbl),"%.4f-%.4f",bins[i].lo,bins[i].hi);
        snprintf(tmp,sizeof(tmp),
            "<row r=\"%d\"><c r=\"A%d\" t=\"inlineStr\"><is><t>%s</t></is></c>"
            "<c r=\"B%d\"><v>%d</v></c>"
            "<c r=\"C%d\" s=\"1\"><v>%.4f</v></c></row>",
            i+2,i+2,lbl,i+2,bins[i].count,i+2,pct);
        sb_append(sb,tmp);
    }
    sb_append(sb,"</sheetData>");
    sb_append(sb,"<drawing r:id=\"rId1\"/>");
    sb_append(sb,"</worksheet>");
}
static void xlsx_sheet2_rels(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">");
    sb_append(sb,"<Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing\" Target=\"../drawings/drawing1.xml\"/>");
    sb_append(sb,"</Relationships>");
}
static void xlsx_drawing(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<xdr:wsDr xmlns:xdr=\"http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing\" xmlns:a=\"http://schemas.openxmlformats.org/drawingml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\">");
    sb_append(sb,"<xdr:absoluteAnchor><xdr:pos x=\"914400\" y=\"0\"/><xdr:ext cx=\"6096000\" cy=\"3810000\"/>");
    sb_append(sb,"<xdr:graphicFrame macro=\"\"><xdr:nvGraphicFramePr><xdr:cNvPr id=\"2\" name=\"Chart 1\"/><xdr:cNvGraphicFramePr/></xdr:nvGraphicFramePr>");
    sb_append(sb,"<xdr:xfrm><a:off x=\"914400\" y=\"0\"/><a:ext cx=\"6096000\" cy=\"3810000\"/></xdr:xfrm>");
    sb_append(sb,"<a:graphic><a:graphicData uri=\"http://schemas.openxmlformats.org/drawingml/2006/chart\">");
    sb_append(sb,"<c:chart xmlns:c=\"http://schemas.openxmlformats.org/drawingml/2006/chart\" r:id=\"rId1\"/>");
    sb_append(sb,"</a:graphicData></a:graphic></xdr:graphicFrame><xdr:clientData/></xdr:absoluteAnchor>");
    sb_append(sb,"</xdr:wsDr>");
}
static void xlsx_drawing_rels(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">");
    sb_append(sb,"<Relationship Id=\"rId1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart\" Target=\"../charts/chart1.xml\"/>");
    sb_append(sb,"</Relationships>");
}
static void xlsx_chart(sb_t *sb) {
    sb_append(sb,"<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\r\n");
    sb_append(sb,"<c:chartSpace xmlns:c=\"http://schemas.openxmlformats.org/drawingml/2006/chart\" xmlns:a=\"http://schemas.openxmlformats.org/drawingml/2006/main\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\">");
    sb_append(sb,"<c:chart>");
    sb_append(sb,"<c:title><c:tx><c:rich><a:bodyPr/><a:lstStyle/><a:p><a:r><a:t>Dot Diameter Distribution</a:t></a:r></a:p></c:rich></c:tx><c:overlay val=\"0\"/></c:title>");
    sb_append(sb,"<c:autoTitleDeleted val=\"0\"/>");
    sb_append(sb,"<c:plotArea><c:barChart>");
    sb_append(sb,"<c:barDir val=\"col\"/><c:grouping val=\"clustered\"/>");
    sb_append(sb,"<c:ser><c:idx val=\"0\"/><c:order val=\"0\"/>");
    sb_append(sb,"<c:tx><c:strRef><c:f>Distribution!$B$1</c:f><c:strCache><c:ptCount val=\"1\"/><c:pt idx=\"0\"><c:v>Count</c:v></c:pt></c:strCache></c:strRef></c:tx>");
    sb_append(sb,"<c:cat><c:strRef><c:f>Distribution!$A$2:$A$21</c:f></c:strRef></c:cat>");
    sb_append(sb,"<c:val><c:numRef><c:f>Distribution!$B$2:$B$21</c:f></c:numRef></c:val>");
    sb_append(sb,"</c:ser>");
    sb_append(sb,"<c:axId val=\"1\"/><c:axId val=\"2\"/>");
    sb_append(sb,"</c:barChart>");
    sb_append(sb,"<c:catAx><c:axId val=\"1\"/><c:scaling><c:orientation val=\"minMax\"/></c:scaling><c:delete val=\"0\"/><c:axPos val=\"b\"/><c:crossAx val=\"2\"/></c:catAx>");
    sb_append(sb,"<c:valAx><c:axId val=\"2\"/><c:scaling><c:orientation val=\"minMax\"/></c:scaling><c:delete val=\"0\"/><c:axPos val=\"l\"/><c:crossAx val=\"1\"/></c:valAx>");
    sb_append(sb,"</c:plotArea>");
    sb_append(sb,"<c:plotVisOnly val=\"1\"/>");
    sb_append(sb,"</c:chart></c:chartSpace>");
}

static void write_xlsx(const char *folder) {
    if (g_xnd == 0) return;
    xbin_t bins[XLSX_BINS];
    calc_hist(bins);

    char path[MAX_PATH_LEN];
    snprintf(path, MAX_PATH_LEN, "%s\\dot_measurements.xlsx", folder);
    FILE *f = fopen(path, "wb");
    if (!f) return;

    zipw_t z; memset(&z,0,sizeof(z)); z.f = f;
    sb_t sb; memset(&sb,0,sizeof(sb));

    sb.len=0; xlsx_content_types(&sb);
    zipw_add(&z, "[Content_Types].xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_root_rels(&sb);
    zipw_add(&z, "_rels/.rels", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_workbook(&sb);
    zipw_add(&z, "xl/workbook.xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_workbook_rels(&sb);
    zipw_add(&z, "xl/_rels/workbook.xml.rels", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_styles(&sb);
    zipw_add(&z, "xl/styles.xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_sheet1(&sb);
    zipw_add(&z, "xl/worksheets/sheet1.xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_sheet2(&sb, bins);
    zipw_add(&z, "xl/worksheets/sheet2.xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_sheet2_rels(&sb);
    zipw_add(&z, "xl/worksheets/_rels/sheet2.xml.rels", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_drawing(&sb);
    zipw_add(&z, "xl/drawings/drawing1.xml", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_drawing_rels(&sb);
    zipw_add(&z, "xl/drawings/_rels/drawing1.xml.rels", (const uint8_t*)sb.p, sb.len);

    sb.len=0; xlsx_chart(&sb);
    zipw_add(&z, "xl/charts/chart1.xml", (const uint8_t*)sb.p, sb.len);
...
static void process_images(void) {
    ...
    fclose(csv);
    write_xlsx(g_folder_a);

    snprintf(g_status_text, sizeof(g_status_text),
        "Done! %d files, %d dots (%d merged, %d missed). CSV + XLSX: %s",
