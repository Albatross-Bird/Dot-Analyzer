/*
 * Dot Analyzer v8.2.11 — ImGui + OpenGL3 Edition
 *
...
static void pstr_diag45(uint8_t *rgb, int x, int y, const char *s,
                        uint8_t r, uint8_t g, uint8_t b) {
    while (*s) { pchar(rgb, x, y, *s, r, g, b); x += PFONT_W + 1; y += PFONT_W + 1; s++; }
}
...
static uint8_t *render_dist_rgb(double bw_override) {
    ...
    /* --- Determine if x-axis labels need diagonal rendering.
       pw is independent of MB (bottom margin), so tilt can be decided before layout. --- */
    const int ML = 72, MR = 24, MT = 36;
    const int pw_fixed = PLOT_W - ML - MR - 1; /* 703 px */
    char tilt_lbl[16]; snprintf(tilt_lbl, sizeof(tilt_lbl), "%.3f", bmin);
    int tilt = (pstr_w(tilt_lbl) + 4 > pw_fixed / nbins);
    const int MB = tilt ? 80 : 52;
    ...
    /* --- X-axis ticks: one label per bin, centered on bar.
       Median bin drawn in red. Labels tilt 45 degrees when bins are too narrow for horizontal text. --- */
    for (int i = 0; i < nbins; i++) {
        ...
        int lw = pstr_w(lbl);
        if (tilt)
            pstr_diag45(rgb, tx - lw / 2, py1 + tick_len + 3, lbl, tr, tg, tb);
        else
