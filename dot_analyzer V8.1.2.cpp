...
static uint8_t *g_stream_preview_buf = NULL;  /* raw frame from stream */
static int      g_stream_preview_w = 0, g_stream_preview_h = 0;
static int      g_stream_preview_active = 0;  /* showing stream preview instead of analysis */
static volatile int g_stream_preview_request = 0; /* request flag for capture thread */
static HANDLE   g_stream_preview_thread = NULL;
...
/* ---- Stream preview capture thread ----
   Captures a single frame from the GVSP stream and posts to UI thread. */
static DWORD WINAPI stream_preview_thread_proc(LPVOID param) {
    (void)param;
    char errbuf[PCAP_ERRBUF_SIZE];
    pcap_t *handle = p_open_live(g_rec_iface, 65535, 1, 10, errbuf);
    if (!handle) {
        snprintf(g_rec_status, sizeof(g_rec_status), "Preview: cannot open interface");
        g_stream_preview_request = 0; return 1;
    }

    /* Discover stream (same as recording thread) */
    char disc_ip[64] = ""; uint16_t disc_port = 0; int found = 0;
    time_t t0 = time(NULL);
    int frame_w = REC_WIDTH, frame_h = REC_HEIGHT;
    while (!found && !g_rec_stop && (time(NULL) - t0 < 10)) {
    ...
    if (!found || g_rec_stop) { p_close(handle); g_stream_preview_request=0; return 1; }
    ...
    uint32_t fsz = frame_w * frame_h;
    uint8_t *fbuf = (uint8_t *)malloc(fsz);
    if (!fbuf) { p_close(handle); g_stream_preview_request=0; return 1; }
    uint32_t cursor=0; uint16_t cur_block=0; int got_frame=0;
    time_t t1=time(NULL);
    while (!got_frame && !g_rec_stop && (time(NULL)-t1<5)) {
        struct pcap_pkthdr *hdr; const unsigned char *pkt;
        int res=p_next_ex(handle,&hdr,&pkt);
        if(res<=0) continue; if(hdr->caplen<50) continue;
        const unsigned char *ip=pkt+14;
    ...
    if (got_frame) {
        ...
        if (tbuf) {
            ...
            if (g_stream_preview_buf) free(g_stream_preview_buf);
            g_stream_preview_buf = tbuf;
            g_stream_preview_w = tw;
            g_stream_preview_h = th;
            LeaveCriticalSection(&g_rec_cs);
            PostMessageW(g_hwnd, WM_STREAM_PREVIEW, 0, 0);
        }
    } else {
        free(fbuf);
        snprintf(g_rec_status, sizeof(g_rec_status), "Preview: no complete frame captured");
    }
    g_stream_preview_request = 0;
    return 0;
}

static void request_stream_preview(void) {
    if (g_stream_preview_request) return; /* already in progress */
    ...
    g_rec_stop = 0;
    g_stream_preview_request = 1;
    snprintf(g_rec_status, sizeof(g_rec_status), "Preview: capturing frame...");
    g_stream_preview_thread = CreateThread(NULL, 0, stream_preview_thread_proc, NULL, 0, NULL);
    if (!g_stream_preview_thread) g_stream_preview_request = 0;
}
...
/* Detect stitch grid dimensions from save timestamps.
   Within a row, column-to-column gaps are short. Row-to-row gaps are long
   (machine traverses back to column 0). Returns 1 on success. */
    ...
    int *out_cols, int *out_rows)
{
    *out_cols = ntiles; *out_rows = 1; /* fallback: single row */
    if (ntiles < 2) return 0;

    /* Compute inter-tile gaps */
    int ng = ntiles - 1;
    ...
    double med = sorted[ng / 2];
    free(sorted);

    /* Threshold: a gap > 2x median is a row break.
       If all gaps are similar (single row), no breaks found. */
    double thresh = med * 2.0;
    if (thresh < 0.5) thresh = 0.5; /* minimum 0.5s to avoid noise */

    /* Find first row break — that gives us column count */
    int first_break = -1;
    for (int i = 0; i < ng; i++) {
        if (gaps[i] > thresh) { first_break = i; break; }
    }
    free(gaps);

    if (first_break < 0) {
        /* No row breaks found — single row */
        *out_cols = ntiles; *out_rows = 1;
        return 1;
    }

    int ncols = first_break + 1; /* tiles before first break = column count */
    if (ncols < 1) ncols = 1;

    /* Validate: total should be roughly divisible by ncols.
       Allow last row to be incomplete. */
...
static void start_recording(void) {
    ...
    /* Close stream preview if active */
    if (g_stream_preview_active) {
        g_stream_preview_active = 0;
        if (g_stream_preview_buf) { free(g_stream_preview_buf); g_stream_preview_buf = NULL; }
    }
...
static void draw_ui(void) {
    ...
    ImGui::BeginChild("##LeftPanel", ImVec2(g_left_panel_w, total_h), true);
    {
        ...
        if (g_rec_state == REC_IDLE && !g_stream_preview_request) {
            if (ImGui::Button("Capture Preview", ImVec2(-1, 0)))
                request_stream_preview();
        } else if (g_stream_preview_request) {
    ...
    ImGui::BeginChild("##RightPanel", ImVec2(0, total_h), true);
    {
        ...
        /* Stream preview indicator and close button */
        if (g_stream_preview_active) {
            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.8f, 0.2f, 1.0f));
            ImGui::Text("STREAM PREVIEW");
            ImGui::PopStyleColor();
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DESTROY:
        ...
        PostQuitMessage(0);
        return 0;
    case WM_STREAM_PREVIEW:
        /* Stream preview frame arrived — convert to RGB and show */
        EnterCriticalSection(&g_rec_cs);
        if (g_stream_preview_buf) {
            int sw = g_stream_preview_w, sh = g_stream_preview_h;
            /* Reuse preview RGB buffer for display */
            if (g_preview_rgb) free(g_preview_rgb);
            g_preview_rgb = (uint8_t *)malloc(sw * sh * 3);
            if (g_preview_rgb) {
                for (int i = 0; i < sw * sh; i++) {
                    uint8_t v = g_stream_preview_buf[i];
                    g_preview_rgb[i*3]=v; g_preview_rgb[i*3+1]=v; g_preview_rgb[i*3+2]=v;
                }
                /* Temporarily set preview image dimensions for texture upload */
                g_preview_img.width = sw; g_preview_img.height = sh;
                g_preview_valid = 1;
                g_stream_preview_active = 1;
                upload_preview_texture();
                snprintf(g_rec_status, sizeof(g_rec_status),
                    "Preview: %dx%d frame captured. Adjust Flip/Rotate if needed.", sw, sh);
            }
        }
        LeaveCriticalSection(&g_rec_cs);
        return 0;
