...
#define MAD_HISTORY_SZ 600   /* ~10 seconds at 60fps */
...
static void start_recording(void) {
    ...
    if (g_live_running || g_stream_preview_active) stop_live_preview();
...
static void draw_ui(void) {
    ...
    ImGui::BeginChild("##RightPanel", ImVec2(0, total_h), true);
    {
        ...
        /* Live feed indicator and close button */
        ...
        /* Preview image */
        float mad_graph_h = g_stream_preview_active ? (120 * g_dpi_scale) : 0;
        if (g_preview_valid && g_preview_tex && g_tex_w > 0) {
            ...
            if (g_stream_preview_active && g_mad_history_count > 1) {
                ImGui::Spacing();
                ImGui::Separator();
                ...
                /* Scale: auto with some headroom, minimum 20 */
                float scale_max = g_mad_peak * 1.2f;
                if (scale_max < 20.0f) scale_max = 20.0f;
